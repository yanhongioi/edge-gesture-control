...
- [x] 手部追蹤：找到手之後，裁切框 = 上一幀骨架範圍 ×1.8 + 移動速度預測，只跑骨架模型；跟丟了才跑 `hand_detect`。模擬測試中，偵測模型有 60% 的幀漏掉手時，`ok` 從約 45% 提升到 100%，手部偵測只剩約 1% 的幀要跑；手離開畫面時不會留下殘影（PC 測試通過，**待上板驗證**）
- [ ] 只在人物附近找手（把人物周圍裁成正方形交給手部偵測，遠距離時手在模型輸入裡會變大）
- [ ] 手勢分類（從 21 個點判斷手勢）
