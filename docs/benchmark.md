...
| NPU 忙碌 % 的意思 | NPU 一次只跑一個推論，三個模型輪流排隊；40% = 一秒裡約 0.4 秒在推論。閒置的 60% 不能「免費」拿來多跑模型：`hand_cam.py` 是 CPU 做完 → NPU → CPU 依序做，多跑一個模型就直接加到每幀時間上（除非改成管線化）。瓶頸在鏡頭 30 fps 和 CPU 前後處理，不在 NPU |
| 換模型的成本 | 實測約 0.03 ms（同一個模型連續跑 9.81 ms vs 三個模型輪流跑 9.85 ms；一輪三個 28.17 ms vs 單獨相加 27.94 ms）。每個模型啟動時就載入好，換模型不會重新載入。推算用的是 `hand_cam.py` 實際跑時的推論時間，已經包含這些開銷 |
